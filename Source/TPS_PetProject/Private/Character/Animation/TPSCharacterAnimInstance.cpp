...
#include "Character/Animation/TPSCharacterAnimInstance.h"
...
void UTPSCharacterAnimInstance::NativeInitializeAnimation()
{
	...
	Character = Cast<ATPS_BaseCharacter>(TryGetPawnOwner());
	if (Character)
	{
		Character->OnJumpedDelegate.AddUniqueDynamic(this, &UTPSCharacterAnimInstance::OnJumped);
	}
}
...
void UTPSCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	LadderSpeedRate = Character->GetMyMovementComponent()->GetLadderSpeedRation();
	...
	bUnderWater = Character->GetSwimmingSystemComponent()->IsUnderWater();
	HorizontalAxis = Character->GetTurnCameraValue();
	VerticalAxis = -Character->GetUpCameraValue();
	Speed = Character->GetMovementComponent()->Velocity.Length();
	bIsLeftSide_WallRun = Character->GetRunnableWallDetectorComponent()->GetISPreviousWallSide() == Left || Character->GetRunnableWallDetectorComponent()->GetISWallSide() == Left;
	// CurrentWallide = Character->GetRunnableWallDetectorComponent()->GetISWallSide();
	// PreviousWallide = Character->GetRunnableWallDetectorComponent()->GetISPreviousWallSide();
	...
	{
		// Check If Moving Or Not & Enable Movement Animations if IsMoving and HasMovementInput, or if the Speed is
		// greater than 150.
...
void UTPSCharacterAnimInstance::PlayTransition(const FTPSDynamicMontageParams &Parameters)
{
	PlaySlotAnimationAsDynamicMontage(Parameters.Animation, NAME_Grounded___Slot, Parameters.BlendInTime,
									  Parameters.BlendOutTime, Parameters.PlayRate, 1, 0.0f, Parameters.StartTime);
}

void UTPSCharacterAnimInstance::PlayTransitionChecked(const FTPSDynamicMontageParams &Parameters)
{
...
void UTPSCharacterAnimInstance::PlayDynamicTransition(float ReTriggerDelay, FTPSDynamicMontageParams Parameters)
{
	if (bCanPlayDynamicTransition)
	{
		...
		World->GetTimerManager().SetTimer(PlayDynamicTransitionTimer, this,
										  &UTPSCharacterAnimInstance::PlayDynamicTransitionDelay, ReTriggerDelay,
										  false);
	}
}

bool UTPSCharacterAnimInstance::ShouldMoveCheck() const
{
	return (CharacterInformation.bIsMoving && CharacterInformation.bHasMovementInput) ||
		CharacterInformation.Speed > 150.0f;
}

bool UTPSCharacterAnimInstance::CanRotateInPlace() const
{
	return RotationMode.Aiming() || CharacterInformation.ViewMode == ETPSViewMode::FirstPerson;
}

bool UTPSCharacterAnimInstance::CanTurnInPlace() const
{
	return RotationMode.LookingDirection() && CharacterInformation.ViewMode == ETPSViewMode::ThirdPerson &&
		GetCurveValue(NAME_Enable_Transition) >= 0.99f;
}

bool UTPSCharacterAnimInstance::CanDynamicTransition() const { return GetCurveValue(NAME_Enable_Transition) >= 0.99f; }

void UTPSCharacterAnimInstance::PlayDynamicTransitionDelay() { bCanPlayDynamicTransition = true; }

void UTPSCharacterAnimInstance::OnJumpedDelay() { InAir.bJumped = false; }

void UTPSCharacterAnimInstance::OnPivotDelay() { Grounded.bPivot = false; }

void UTPSCharacterAnimInstance::UpdateAimingValues(float DeltaSeconds)
{
	...
	AimingValues.SmoothedAimingRotation =
		FMath::RInterpTo(AimingValues.SmoothedAimingRotation, CharacterInformation.AimingRotation, DeltaSeconds,
	...
	if (!RotationMode.VelocityDirection())
	{
		...
		AimingValues.AimSweepTime =
			FMath::GetMappedRangeValueClamped<float, float>({-90.0f, 90.0f}, {1.0f, 0.0f}, AimingValues.AimingAngle.Y);
	...
	else if (CharacterInformation.bHasMovementInput)
	{
		...
		const float InterpTarget =
			FMath::GetMappedRangeValueClamped<float, float>({-180.0f, 180.0f}, {0.0f, 1.0f}, Delta.Yaw);

		AimingValues.InputYawOffsetTime = FMath::FInterpTo(AimingValues.InputYawOffsetTime, InterpTarget, DeltaSeconds,
	...
	AimingValues.ForwardYawTime =
		FMath::GetMappedRangeValueClamped<float, float>({-180.0f, 180.0f}, {0.0f, 1.0f}, SmoothedAimingAngle.X);
}

void UTPSCharacterAnimInstance::UpdateLayerValues()
{
	...
	LayerBlendingValues.EnableHandIK_L =
		FMath::Lerp(0.0f, GetCurveValue(NAME_Enable_HandIK_L), GetCurveValue(NAME_Layering_Arm_L));
	LayerBlendingValues.EnableHandIK_R =
		FMath::Lerp(0.0f, GetCurveValue(NAME_Enable_HandIK_R), GetCurveValue(NAME_Layering_Arm_R));
...
void UTPSCharacterAnimInstance::UpdateFootIK(float DeltaSeconds)
{
	FVector FootOffsetLTarget = FVector::ZeroVector;
	FVector FootOffsetRTarget = FVector::ZeroVector;

	// Update Foot Locking values.
	SetFootLocking(DeltaSeconds, NAME_Enable_FootIK_L, NAME_FootLock_L, IkFootL_BoneName, FootIKValues.FootLock_L_Alpha,
				   FootIKValues.UseFootLockCurve_L, FootIKValues.FootLock_L_Location, FootIKValues.FootLock_L_Rotation);
	SetFootLocking(DeltaSeconds, NAME_Enable_FootIK_R, NAME_FootLock_R, IkFootR_BoneName, FootIKValues.FootLock_R_Alpha,
				   FootIKValues.UseFootLockCurve_R, FootIKValues.FootLock_R_Location, FootIKValues.FootLock_R_Rotation);
...
void UTPSCharacterAnimInstance::SetFootLocking(float DeltaSeconds, FName EnableFootIKCurve, FName FootLockCurve,
											   ...
											   FVector &CurFootLockLoc, FRotator &CurFootLockRot)
{
	...
	if (UseFootLockCurve)
	{
		...
		FootLockCurveVal =
			GetCurveValue(FootLockCurve) * (1.f / GetSkelMeshComponent()->AnimUpdateRateParams->UpdateRate);
	}
	else
	{
	...
	// Step 3: If the Foot Lock curve equals 1, save the new lock location and rotation in component space as the
	// target.
...
void UTPSCharacterAnimInstance::SetFootLockOffsets(float DeltaSeconds, FVector &LocalLoc, FRotator &LocalRot)
{
	...
	{
		RotationDifference =
			CharacterInformation.CharacterActorRotation - Character->GetCharacterMovement()->GetLastUpdateRotation();
...
void UTPSCharacterAnimInstance::SetPelvisIKOffset(float DeltaSeconds, FVector FootOffsetLTarget,
...
void UTPSCharacterAnimInstance::ResetIKOffsets(float DeltaSeconds)
{
	// Interp Foot IK offsets back to 0
	FootIKValues.FootOffset_L_Location =
		FMath::VInterpTo(FootIKValues.FootOffset_L_Location, FVector::ZeroVector, DeltaSeconds, 15.0f);
	FootIKValues.FootOffset_R_Location =
		FMath::VInterpTo(FootIKValues.FootOffset_R_Location, FVector::ZeroVector, DeltaSeconds, 15.0f);
	FootIKValues.FootOffset_L_Rotation =
		FMath::RInterpTo(FootIKValues.FootOffset_L_Rotation, FRotator::ZeroRotator, DeltaSeconds, 15.0f);
	FootIKValues.FootOffset_R_Rotation =
		FMath::RInterpTo(FootIKValues.FootOffset_R_Rotation, FRotator::ZeroRotator, DeltaSeconds, 15.0f);
}

void UTPSCharacterAnimInstance::SetFootOffsets(float DeltaSeconds, FName EnableFootIKCurve, FName IKFootBone,
											   ...
											   FRotator &CurRotationOffset)
{
	...
	// {
	// 	UALSDebugComponent::DrawDebugLineTraceSingle(World, TraceStart, TraceEnd, EDrawDebugTrace::Type::ForOneFrame,
	...
	if (Character->GetCharacterMovement()->IsWalkable(HitResult))
	{
		...
		CurLocationTarget =
			(ImpactPoint + ImpactNormal * Config.FootHeight) - (IKFootFloorLoc + FVector(0, 0, Config.FootHeight));
...
void UTPSCharacterAnimInstance::RotateInPlaceCheck()
{
...
void UTPSCharacterAnimInstance::TurnInPlaceCheck(float DeltaSeconds)
{
	// Step 1: Check if Aiming angle is outside of the Turn Check Min Angle, and if the Aim Yaw Rate is below the Aim
	// Yaw Rate Limit. If so, begin counting the Elapsed Delay Time. If not, reset the Elapsed Delay Time. This ensures
	// the conditions remain true for a sustained period of time before turning in place.
	...
	const float ClampedAimAngle = FMath::GetMappedRangeValueClamped<float, float>(
		...
		{TurnInPlaceValues.MinAngleDelay, TurnInPlaceValues.MaxAngleDelay}, AimingValues.AimingAngle.X);

	// Step 2: Check if the Elapsed Delay time exceeds the set delay (mapped to the turn angle range). If so, trigger a
	// Turn In Place.
...
void UTPSCharacterAnimInstance::DynamicTransitionCheck()
{
	// Check each foot to see if the location difference between the IK_Foot bone and its desired / target location
	// (determined via a virtual bone) exceeds a threshold. If it does, play an additive transition animation on that
	// foot. The currently set transition plays the second half of a 2 foot transition animation, so that only a single
	// foot moves. Because only the IK_Foot bone can be locked, the separate virtual bone allows the system to know its
	...
	{
		FTPSDynamicMontageParams Params;
...
void UTPSCharacterAnimInstance::UpdateMovementValues(float DeltaSeconds)
{
	// Interp and set the Velocity Blend.
	const FTPSVelocityBlend &TargetBlend = CalculateVelocityBlend();
	...
	LeanAmount.LR =
		FMath::FInterpTo(LeanAmount.LR, RelativeAccelerationAmount.Y, DeltaSeconds, Config.GroundedLeanInterpSpeed);
	LeanAmount.FB =
		FMath::FInterpTo(LeanAmount.FB, RelativeAccelerationAmount.X, DeltaSeconds, Config.GroundedLeanInterpSpeed);
...
void UTPSCharacterAnimInstance::UpdateRotationValues()
{
...
void UTPSCharacterAnimInstance::UpdateInAirValues(float DeltaSeconds)
{
	// Update the fall speed. Setting this value only while in the air allows you to use it within the AnimGraph for the
	// landing strength. If not, the Z velocity would return to 0 on landing.
	...
	const FTPSLeanAmount &InAirLeanAmount = CalculateAirLeanAmount();
...
void UTPSCharacterAnimInstance::UpdateRagdollValues()
{
	...
	const float VelocityLength =
		GetOwningComponent()->GetPhysicsLinearVelocity(NAME__ALSCharacterAnimInstance__root).Size();
	FlailRate = FMath::GetMappedRangeValueClamped<float, float>({0.0f, 1000.0f}, {0.0f, 1.0f}, VelocityLength);
}

float UTPSCharacterAnimInstance::GetAnimCurveClamped(const FName &Name, float Bias, float ClampMin,
													 float ClampMax) const
{
	return FMath::Clamp(GetCurveValue(Name) + Bias, ClampMin, ClampMax);
}

FTPSVelocityBlend UTPSCharacterAnimInstance::CalculateVelocityBlend() const
{
	// Calculate the Velocity Blend. This value represents the velocity amount of the actor in each direction
	// (normalized so that diagonals equal .5 for each direction), and is used in a BlendMulti node to produce better
...
FVector UTPSCharacterAnimInstance::CalculateRelativeAccelerationAmount() const
{
	// Calculate the Relative Acceleration Amount. This value represents the current amount of acceleration /
	// deceleration relative to the actor rotation. It is normalized to a range of -1 to 1 so that -1 equals the Max
...
float UTPSCharacterAnimInstance::CalculateStrideBlend() const
{
...
float UTPSCharacterAnimInstance::CalculateWalkRunBlend() const
{
...
float UTPSCharacterAnimInstance::CalculateStandingPlayRate() const
{
	...
	// The value is also divided by the Stride Blend and the mesh scale so that the play rate increases as the stride or
	// scale gets smaller
...
float UTPSCharacterAnimInstance::CalculateDiagonalScaleAmount() const
{
...
float UTPSCharacterAnimInstance::CalculateCrouchingPlayRate() const
{
	// Calculate the Crouching Play Rate by dividing the Character's speed by the Animated Speed.
	// This value needs to be separate from the standing play rate to improve the blend from crouch to stand while in
	// motion.
	return FMath::Clamp(CharacterInformation.Speed / Config.AnimatedCrouchSpeed / Grounded.StrideBlend /
							GetOwningComponent()->GetComponentScale().Z,
						0.0f, 2.0f);
}

float UTPSCharacterAnimInstance::CalculateLandPrediction() const
{
	// Calculate the land prediction weight by tracing in the velocity direction to find a walkable surface the
	// character is falling toward, and getting the 'Time' (range of 0-1, 1 being maximum, 0 being about to land) till
	// impact. The Land Prediction Curve is used to control how the time affects the final weight for a smooth blend.
	...
	const FVector TraceLength = VelocityClamped *
		FMath::GetMappedRangeValueClamped<float, float>({0.0f, -4000.0f}, {50.0f, 2000.0f}, VelocityZ);
	...
	const bool bHit = World->SweepSingleByChannel(HitResult, CapsuleWorldLoc, CapsuleWorldLoc + TraceLength,
												  FQuat::Identity, ECC_Visibility, CapsuleCollisionShape, Params);

	// if (ALSDebugComponent && ALSDebugComponent->GetShowTraces())
	// {
	// 	UALSDebugComponent::DrawDebugCapsuleTraceSingle(World, CapsuleWorldLoc, CapsuleWorldLoc + TraceLength,
	// 													CapsuleCollisionShape, EDrawDebugTrace::Type::ForOneFrame, bHit,
	// 													HitResult, FLinearColor::Red, FLinearColor::Green, 5.0f);
	// }
...
FTPSLeanAmount UTPSCharacterAnimInstance::CalculateAirLeanAmount() const
{
	...
	FTPSLeanAmount CalcLeanAmount;
	const FVector &UnrotatedVel =
		CharacterInformation.CharacterActorRotation.UnrotateVector(CharacterInformation.Velocity) / 350.0f;
...
ETPSMovementDirection UTPSCharacterAnimInstance::CalculateMovementDirection() const
{
	...
	FRotator Delta = CharacterInformation.Velocity.ToOrientationRotator() - CharacterInformation.AimingRotation;
	Delta.Normalize();
	return UTPSMathLibrary::CalculateQuadrant(MovementDirection, 70.0f, -70.0f, 110.0f, -110.0f, 5.0f, Delta.Yaw);
}

void UTPSCharacterAnimInstance::TurnInPlace(FRotator TargetRotation, float PlayRateScale, float StartTime,
											bool OverrideCurrent)
{
	...
	FTPSTurnInPlaceAsset TargetTurnAsset;
	if (Stance.Standing())
	{
		...
		{
			TargetTurnAsset = TurnAngle < 0.0f ? TurnInPlaceValues.N_TurnIP_L180 : TurnInPlaceValues.N_TurnIP_R180;
		}
	}
	else
	{
	...
	// Step 3: If the Target Turn Animation is not playing or set to be overriden, play the turn animation as a dynamic
	// montage.
	...
	// Step 4: Scale the rotation amount (gets scaled in AnimGraph) to compensate for turn angle (If Allowed) and play
	// rate.
...
void UTPSCharacterAnimInstance::OnJumped()
{
	InAir.bJumped = true;
	InAir.JumpPlayRate =
		FMath::GetMappedRangeValueClamped<float, float>({0.0f, 600.0f}, {1.2f, 1.5f}, CharacterInformation.Speed);

	GetWorld()->GetTimerManager().SetTimer(OnJumpedTimer, this, &UTPSCharacterAnimInstance::OnJumpedDelay, 0.1f, false);
}

void UTPSCharacterAnimInstance::OnPivot()
{
	Grounded.bPivot = CharacterInformation.Speed < Config.TriggerPivotSpeedLimit;
	GetWorld()->GetTimerManager().SetTimer(OnPivotTimer, this, &UTPSCharacterAnimInstance::OnPivotDelay, 0.1f, false);
}
